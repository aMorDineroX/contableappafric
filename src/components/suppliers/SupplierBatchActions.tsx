...
const SupplierBatchActions: React.FC<SupplierBatchActionsProps> = ({
  selectedCount,
  onBatchDelete,
  onBatchStatusChange,
  onClearSelection
      ...
      <div className="container mx-auto flex items-center justify-between">
        <div className="flex items-center">
          <span className="text-sm font-medium text-gray-700 mr-4">
            {selectedCount} fournisseur{selectedCount > 1 ? 's' : ''} sélectionné{selectedCount > 1 ? 's' : ''}
          </span>
          <button
        ...
        <div className="flex space-x-2">
          ...
          <button
            onClick={() => {
              if (window.confirm(`Êtes-vous sûr de vouloir supprimer ${selectedCount} fournisseur${selectedCount > 1 ? 's' : ''} ?`)) {
                onBatchDelete();
              }
            }}
...
export default SupplierBatchActions;
